...
public:
    ...
    void sendUpdatePasswordToAll() {
        list<uint32_t> targets = mesh->getNodeList();
        Scene passwordUpdateScene = findPasswordUpdateScene();
        for(uint32_t target : targets) {
            sendEventToExecutor(target, passwordUpdateScene.executor, mesh_password);
        }
        Serial.println();
    }

    Scene findPasswordUpdateScene() {
        for(Scene scene : scenes) {
            if(scene.event.equals("change.password")) {
                return scene;
            }
        }
        return Scene();
